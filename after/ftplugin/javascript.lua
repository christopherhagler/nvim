...
-- We use npx to ensure we use the project's local eslint version if available
local lint_cmd = string.format("npx eslint %s", vim.fn.expand("%"))

-- Run command: Execute the script with node
local run_cmd = string.format("node %s", vim.fn.expand("%"))
